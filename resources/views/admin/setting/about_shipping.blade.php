    ...
    <main class="c-main">
        <div class="container-fluid">
            <div class="fade-in">
                <x-alert />
                <div class="row">
                    {{ html()->form('POST', route('admin.setting.updateAboutShipping.post'))->acceptsFiles()->id('about_shipping')->open() }}
                    <div class="col-sm-12">
                        <div class="card mb-3">
                            ...
                            <div class="card-body table-listing table-responsive">
                                <div class="row product-field-wrapper">
                                    <div class="col-md-12">
                                        {{ html()->label('Banner') }}
                                        {{ html()->file('shipping_banner')->accept('image/')->class('form-control')->required() }}
                                        <br />
                                        <img class="img-fluid" {{ isset($setting_model) ? 'src=' . $setting_model['shipping_banner'] : '' }} />
                                    </div>
                                    <div class="col-md-12 mt-3">
                                        {{ html()->label('Chinese Shipping Info') }}
                                        {{ html()->textarea('cn_shipping_info')->value(isset($setting_model) ? $setting_model['cn_shipping_info'] : '')->class('form-control wysiwyg') }}
                                    </div>
                                    <div class="col-md-12 mt-3">
                                        {{ html()->label('English Shipping Info') }}
                                        {{ html()->textarea('en_shipping_info')->value(isset($setting_model) ? $setting_model['en_shipping_info'] : '')->class('form-control wysiwyg') }}
                                    </div>
                                </div>
                            </div>
                        </div>
