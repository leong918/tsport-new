    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-3">
                        {{ html()->label('Match Title') }}
                        {{ html()->text('match_title')->placeholder('Enter Match Title')->class('form-control')->required() }}
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="mb-3">
                        {{ html()->label('Short Content') }}
                        {{ html()->textarea('short_content')->placeholder('Enter Short Content/Description')->class('form-control')->rows(3) }}
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <div class="input-group datePicker" data-td-target-input="nearest"
                            ...
                            <input type="text" class="form-control" name="start_at" data-td-toggle="datetimepicker"
                                value="{{ old('start_at', isset($model) ? $model->start_at : '') }}"
                                autocomplete="off" />
                        </div>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        {{ html()->label('Status') }}
                        {{ html()->select('status')->options(renderSelect(Matches::STATUS))->class('form-control') }}
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="mb-3">
                        <div class="form-check">
                            {{ html()->checkbox('is_top', isset($model) ? $model->is_top : false, '1')->class('form-check-input')->id('is_top') }}
                            {{ html()->label('Show in Top Carousel', 'is_top')->class('form-check-label') }}
                        </div>
                    </div>
                </div>
                <div class="col-md-12">
                    <div class="mb-3 banner-image" id="banner-image">
                        {{ html()->label('Banner') }}
                        {{ html()->file('banner')->accept('image/*')->class('form-control image-uploader' . (isset($model) && $model->banner ? '' : ' requiredInput')) }}
                        <small class="text-danger errorMessage"></small>
                        {{ html()->hidden('original_banner')->value(isset($model) && $model->banner ? $model->banner : '') }}
                        <div class="image-preview-container"></div>
                        @if (isset($model) && $model->banner)
                            <div class="mb-3">
                                <label>Original Banner Preview</label>
                                <div class="col-3 d-flex justify-content-center">
                                    <img class="img-fluid"
                                        src="{{ $model->banner_url ?? asset('storage/' . $model->banner) }}" 
                                        alt="Banner preview" />
                                </div>
                            </div>
                        @endif
