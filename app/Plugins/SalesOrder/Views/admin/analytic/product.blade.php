                        ...
                        <div class="card">
                            <div class="menu">
                                <ul class="summary d-flex p-0 border-bottom border-1" style="list-style: none">
                                    <li class="col-sm-4 product_sold_sorting border-end border-1 active" data-sort-by="2">
                                        <a href="#" class="text-decoration-none p-4 d-flex flex-column text-reset">
                                            ...
                                            <div class="product_sold_data d-flex">
                                                <div class="product_sold_total col-sm-10"><span style="font-size: 20px"></span></div>
                                                <div class="product_sold_percentage percentage text-center fw-medium rounded pb-1 col-sm-2"><span style="font-size: 0.8rem"></span></div>
                                            </div>
                                        </a>
                                    </li>
                                    <li class="col-sm-4 net_sales_sorting border-end border-1" data-sort-by="3">
                                        <a href="#" class="text-decoration-none p-4 d-flex flex-column text-reset">
                                            <div class="net_sales_label mb-3">
                                                <span style="font-size: 14px">Net Sales</span>
                                            </div>
                                            <div class="net_sales_data d-flex">
                                                <div class="net_sales_total col-sm-10"><span style="font-size: 20px"></span></div>
                                                <div class="net_sales_percentage percentage text-center fw-medium rounded pb-1 col-sm-2"><span style="font-size: 0.8rem"></span></div>
                                            </div>
                                        </a>
                                    </li>
                                    <li class="col-sm-4 orders_sorting" data-sort-by="4">
                                        <a href="#" class="text-decoration-none p-4 d-flex flex-column text-reset">
                                            <div class="orders_label mb-3">
                                                <span style="font-size: 14px">Orders</span>
                                            </div>
                                            <div class="orders_data d-flex">
                                                <div class="orders_total col-sm-10"><span style="font-size: 20px"></span></div>
                                                <div class="orders_percentage percentage text-center fw-medium rounded pb-1 col-sm-2"><span style="font-size: 0.8rem"></span></div>
                                            </div>
                                        </a>
                                    </li>
                                </ul>
    ...
    $(document).ready(function() {
        ...
        $(function() {
            var table = $('.table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{!! route('admin.analytic.product') !!}',
                    data: function (d) {
                        var form = {};
            ...
            $('.summary li').on('click', function(e) {
                ...
                $('.summary li').css({'box-shadow': 'none', 'background-color' : '#f8f9fa'}); 
                $('.summary li.active').css({'box-shadow': 'inset 0 4px 0 #007cba', 'background-color' : '#FFFFFF'}); 
                
                getProductDataByDateRange(sort_by);
        ...
        function getProductDataByDateRange(sort_by) {
            axios({
                method: "post",
                url: '{!! route('admin.analytic.getProductDataByDateRange') !!}',
                data: function () {
                    var form = {};
                    $.each($('#form').serializeArray(), function() {
                        form[this.name] = this.value;
            ...
            .then(response => {
                var product_data_current = response.data.product_data_current;
                var product_data_previous = response.data.product_data_previous;

                updateLineChart(product_data_current, product_data_previous, sort_by);
                updateProductSummary(product_data_current, product_data_previous);
            })
            .catch(error => {
                swal.fire({
                    title: 'Fail',
        ...
        //--------------------------- update line chart ------------------------------- 
        function updateLineChart(product_data_current, product_data_previous, sort_by = null) {
            ...
            for (let j = 1; j <= dayOfMonth.length; j++) {
                ...
                product_data_current.forEach(element => {
                    ...
                    if (compareDate === j) {
                        ...
                        if(sort_by && sort_by > 2){
                            sort_by == 3 ? currentDatasetLabel.push(parseFloat(element['net_sales'])) : 
            ...
            for (let k = 1; k <= dayOfMonth.length; k++) {
                ...
                product_data_previous.forEach(element => {
                    ...
                    if (compareDate === k) {
                        if(sort_by && sort_by > 2){
                            sort_by == 3 ? previousDatasetLabel.push(parseFloat(element['net_sales'])) : 
            ...
            if(moment(end_date).format('MMMM') != moment(start_date).format('MMMM')){
                ...
                if(moment(end_date).format('YYYY') != moment(start_date).format('YYYY')){
                    console.log('here');
                    currentLabel = moment(start_date).format('MMMM YYYY') + ' - ' + moment(end_date).format('MMMM YYYY');
                    previousLabel = moment(start_date).subtract(1, "year").format('MMMM YYYY') + ' - ' + previousLabel;
                }
            }
        ...
        function updateProductSummary(product_data, product_data_previous) {
            var total_product_sold = 0;
            var total_net_sales = 0;
            var total_orders = 0;
            var total_product_data_previous = 0;
            ...
            product_data_previous.forEach(element => {
                total_product_data_previous += parseInt(element['product_sold']);
                total_net_sales_previous += parseFloat(element['net_sales']);
                total_orders_previous += parseInt(element['orders']);
            });

            var product_sold_percent = ((total_product_sold - total_product_data_previous)/total_product_data_previous) * 100;
            ...
            $('.percentage span').each(function() {
                ...
                if($.isNumeric(value)){
                    value > 0 ? $(this).parent().removeClass('bg-success bg-danger').addClass('bg-success') : $(this).parent().removeClass('bg-success bg-danger').addClass('bg-danger');
                } else {
            ...
            $('.summary li').each(function () {
                $(this).hasClass('active') ? $(this).css('box-shadow', 'inset 0 4px 0 #007cba') : $(this).css('background-color', '#f8f9fa');
            });
        }
    });
</script>
